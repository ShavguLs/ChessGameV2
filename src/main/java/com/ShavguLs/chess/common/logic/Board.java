...
public class Board {
    ...
    public boolean attemptMove(int srcRow, int srcCol, int destRow, int destCol, boolean isWhiteTurn, Piece promotionPiece) {
        ...
        Piece pieceAtDest = getPieceAt(destRow, destCol);

        // --- Basic Validation ---
        if (pieceToMove == null || pieceToMove.isWhite() != isWhiteTurn) {
            return false;
        }
        // A king can never be the destination of a capture.
        if (pieceAtDest instanceof King) {
            return false;
        }
        ...
        // --- Standard Move and Safety Validation ---
        if (!pieceToMove.isValidMove(srcRow, srcCol, destRow, destCol, this.board)) {
            return false;
        }
        if (pieceToMove instanceof King && isSquareAttacked(destRow, destCol, !isWhiteTurn)) {
            return false;
        }
        ...
        boolean isPromotionMove = pieceToMove instanceof Pawn && ((pieceToMove.isWhite() && destRow == 0) || (!pieceToMove.isWhite() && destRow == 7));

        // If this IS a promotion, a promotion piece MUST be provided. Otherwise, the move is invalid.
        if (isPromotionMove && promotionPiece == null) {
            return false;
        }
        // --- END CORRECTION ---
        ...
        // If it was a promotion, place the chosen piece on the board.
        if (isPromotionMove) {
            setPiece(destRow, destCol, promotionPiece);
        }

        updateLastMove(srcRow, srcCol, destRow, destCol);
        this.isWhiteTurn = !this.isWhiteTurn; // Flip the turn
        return true;
    }
